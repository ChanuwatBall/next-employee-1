...
const SellTicket: React.FC = () => {
  ...
  const [salePaymentMethod, setSalePaymentMethod] =
    useState<SalePaymentMethod>(null);
  ...
  useEffect(() => {
    const fetchSaleContext = async () => {
      ...
      try {
        ...
        iontoast({
          message: "โหลดข้อมูลเตรียมขายไม่สำเร็จ",
          duration: 2200,
          color: "danger",
          position: "top",
        });
  ...
  const getBoardingPointId = () => {
    ...
    return (
      stops[0]?.id ||
      trip?.route_id?.origin_id ||
      trip?.origin_province_id ||
      ""
    );
  };
  ...
  const getDropOffPointId = () => {
    ...
    return (
      ...
      trip?.route_id?.destination_id ||
      trip?.destination_province_id ||
      ""
    );
  };
  ...
  const validateSaleForm = () => {
    if (saleSeats.length === 0) {
      iontoast({
        message: "ไม่พบที่นั่งสำหรับขายตั๋ว",
        duration: 2200,
        color: "warning",
        position: "top",
      });
    ...
    const missingName = salePassengers.some(
  ...
  const updateSalePassenger = (
    index: number,
    field: keyof SalePassengerForm,
    value: string,
  ) => {
  ...
  const buildBookingPayload = () => ({
    ...
    originProvinceId:
      trip?.origin_province_id || trip?.route_id?.origin_id || "",
    destinationProvinceId:
      trip?.destination_province_id || trip?.route_id?.destination_id || "",
  ...
  const buildReceiptData = (
    ...
    const seats = detail?.seats?.length
      ? detail.seats
      : passengers.map((passenger) => passenger.seatNumber);
    ...
    return {
      ...
      pricePerSeat:
        detail?.pricePerSeat ||
        Math.round(total / Math.max(passengers.length, 1)),
    };
  };
  ...
  const downloadCurrentReceipt = async () => {
    ...
    try {
      ...
      iontoast({
        message: "สร้างใบเสร็จ PDF ไม่สำเร็จ",
        duration: 2200,
        color: "danger",
        position: "top",
      });
  ...
  const setSuccessFromBookingDetail = async (
    ...
    const bookingReference =
      detail?.bookingReference || fallbackReference || saleBookingReference;
  ...
  const startQrPayment = async () => {
    ...
    try {
      const booking: CreateBookingResponse = await createBooking(
        buildBookingPayload(),
      );
      console.log("Booking created:", JSON.stringify(booking));
      const bookingId = booking?.bookingId || extractBookingId(booking);
      if (!bookingId) throw new Error("ไม่พบรหัส booking จากระบบ");
      setSaleBookingId(bookingId);
      setSaleBookingReference(
        booking?.bookingReference || extractBookingReference(booking),
      );
      ...
      const expiresAt = payment?.expiresAt
        ...
        : NaN;
      qrDeadlineRef.current = Number.isFinite(expiresAt)
        ? expiresAt
        : Date.now() + 5 * 60 * 1000;
      qrPollCountRef.current = 0;
      setQrCountdown(
        Math.max(Math.ceil((qrDeadlineRef.current - Date.now()) / 1000), 0),
      );
      setSaleStep("qrcode");
    } catch (err: any) {
  ...
  const handleConfirmCashPayment = async () => {
    if (cashReceivedAmount < saleTotalPrice) {
      iontoast({
        message: "จำนวนเงินสดไม่พอสำหรับยอดชำระ",
        duration: 2200,
        color: "warning",
        position: "top",
      });
    ...
    try {
      ...
      const result: DriverSellTicketResponse = await driverSellTicket(
        payload,
      ...
      const detail = await getDriverBookingDetail(
        result.bookingId,
        session?.access_token,
      );
      await setSuccessFromBookingDetail(
        detail,
        "cash",
        result.bookingReference,
      );
      iontoast({
        message: "ชำระเงินสดสำเร็จ",
        duration: 2000,
        color: "success",
        position: "top",
      });
  ...
  const finalizeQrBookingSuccess = async () => {
    try {
      ...
      const detail = await getDriverBookingDetail(saleBookingId);
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(async () => {
      ...
      try {
        const transaction = await getDriverBookingPayment(qrTransactionId);
        const status =
          `${transaction?.status || transaction?.paymentStatus || transaction?.payment_status || ""}`.toLowerCase();
  ...
  const bookingSeats = bookingDetail?.seats?.length
    ? bookingDetail.seats
    : saleSummaryItems.map((item) => item.passenger.seatNumber);
  const bookingPassengers = bookingDetail?.passengers?.length
        ...
        <div className="sale-modal sell-ticket-page">
          <div className="sale-modal-header">
            <div>
              ...
              <p>
                {salePassengers.length} ผู้โดยสาร • ที่นั่ง{" "}
                {salePassengers.map((p) => p.seatNumber).join(", ") || "-"}
              </p>
            </div>
          </div>
            ...
            <>
                  ...
                  >
                    ...
                    <IonItem className="sale-form-item">
                      <IonLabel position="stacked">
                        เบอร์โทร
                        {useSamePhone && index > 0 ? " (ใช้ร่วมกัน)" : ""}
                      </IonLabel>
                      <IonInput
              ...
              <div className="sale-payment-footer">
                <IonButton
                  ...
                  mode="ios"
                  color="primary"
                  onClick={() => validateSaleForm() && setSaleStep("summary")}
                >
                  ถัดไป
                </IonButton>
              </div>
            ...
            <>
              <div className="sale-summary-card">
                ...
                <div className="sale-summary-row">
                  ...
                  <strong>
                    {saleSummaryItems
                      .map((item) => item.passenger.seatNumber)
                      .join(", ")}
                  </strong>
                </div>
              </div>
              ...
              <div className="sale-summary-list">
                {saleSummaryItems.map((item, index) => (
                  <div
                    className="sale-summary-passenger"
                    key={item.passenger.seatNumber}
                  >
                    <div className="sale-passenger-title">
                      <span>ผู้โดยสาร {index + 1}</span>
                    ...
                    <div className="sale-summary-row">
                      <span>ประเภท</span>
                      <strong>
                        {getPassengerTypeLabel(item.passenger.passengerType)}
                      </strong>
                    </div>
                    <div className="sale-summary-row">
                      <span>ราคา</span>
            ...
            <>
              ...
              <IonItem className="sale-form-item ion-margin-bottom">
                ...
                <IonInput
                  ...
                  min="0"
                  onIonInput={(event) =>
                    setCashReceived(`${event.detail.value || ""}`)
                  }
                />
              </IonItem>
              ...
              <div className="sale-summary-card">
                ...
                <div className="sale-summary-row">
                  ...
                  <strong>
                    ...
                    {(qrCountdown % 60).toString().padStart(2, "0")} นาที
                  </strong>
                </div>
              </div>
          ...
          {saleStep === "success" && (
            <>
              <div className="sale-success-card">
                ...
                {ticketQrCodeImage && (
                  <img
                    className="sale-ticket-qr-image"
                    src={ticketQrCodeImage}
                    alt="QR Code ตั๋ว"
                  />
                )}
              </div>
              ...
              <div className="sale-summary-card">
                <div className="sale-summary-row">
                  <span>เส้นทาง</span>
                  <strong>
                    {bookingDetail?.origin || trip?.route_id?.origin || "-"} -{" "}
                    {bookingDetail?.destination ||
                      trip?.route_id?.destination ||
                      "-"}
                  </strong>
                </div>
                <div className="sale-summary-row">
                  ...
                  <strong>
                    {bookingDetail?.date
                      ? moment(bookingDetail.date).format("DD MMM YYYY")
                      : trip?.date
                        ? moment(trip.date).format("DD MMM YYYY")
                        : "-"}
                    {" • "}
                    {bookingDetail?.departureTime ||
                      trip?.departure_time ||
                      "-"}
                  </strong>
                </div>
                ...
                <div className="sale-summary-row">
                  ...
                  <strong>
                    {bookingDetail?.paymentMethod ||
                      normalizePaymentMethod(salePaymentMethod)}
                  </strong>
                </div>
                <div className="sale-summary-row">
              ...
              <div>
                <IonButton
                  className="ioin-margin-bottom"
                  expand="block"
                  mode="ios"
                  color="primary"
                  onClick={openSoldTicketDetail}
                >
                  เปิดรายละเอียดตั๋ว
                </IonButton>
                <IonButton
                ...
                <IonButton
                  ...
                  fill="outline"
                  color="medium"
                  onClick={() => history.replace(`/plan/${id}`)}
                >
                  กลับไปผังที่นั่ง
                </IonButton>
              </div>
          ...
          {saleStep === "failed" && (
            <>
              <div className="sale-failed-card">
                ...
                <p>
                  {saleErrorMessage || "ไม่พบสถานะ success จากระบบชำระเงิน"}
                </p>
              </div>
